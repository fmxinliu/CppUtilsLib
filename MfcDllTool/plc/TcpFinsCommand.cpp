...
bool TcpFinsCommand::NodeAddressDataSend()
{
    ...
    // send NADS command
    //
    pTransport->Send(cmdNADS, sizeof(cmdNADS));

    // wait for a plc response
    //
    uint8_t respNADS[24] = { 0 };
    pTransport->Receive(respNADS, sizeof(respNADS));
...
bool TcpFinsCommand::FrameSend(const vector<uint8_t> &data)
{
    ...
    // send frame header
    //
    pTransport->Send(&cmdFS[0], cmdFS.size());

    // send FINS command
    //
    pTransport->Send(&cmdFins[0], finsCommandLen);

    // send additional data
    //
    if (data.size() > 0)
    {
        pTransport->Send(&data[0], data.size());
    }

    // frame response
    //
    pTransport->Receive(&respFS[0], respFS.size());
    ...
    //
    pTransport->Receive(&respFins[0], 14);
    ...
    if (finsResponseLen > 14)
    {
        ...
        //
        pTransport->Receive(&respFinsData[0], finsResponseLen - 14);
    }
